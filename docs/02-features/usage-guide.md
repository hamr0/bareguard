...
**4a. Cost contract — "unpriced" ≠ free (v0.9).** When your metered runner can't compute a round's cost (no model on the result, no rate-table entry), forward `{ costUsd: null, pricing: "unpriced" }` to `gate.record` — **not** `costUsd: 0`. An unpriced round accrues no cost (unknown ≠ free, so it can't silently pass a `maxCostUsd` cap), still accrues `tokens`, and emits a loud `phase: "unpriced"` audit line. The gate also derives "unpriced" defensively from the *value* (a present-but-non-finite `costUsd`), so a dropped flag still fails safe. Opt into hard enforcement with `budget: { failClosedOnUnpriced: true }` — under a finite `maxCostUsd` cap, an unpriced round then halts (`rule: "budget.unpriced"`) rather than leaving the cost axis unenforceable. Default is observe-only (the audit line, no halt). Negative `costUsd`/`tokens` are clamped to 0 (spend is monotonic — you can't un-spend past a cap).
...
See also [harness-research.md, Part III "Identity and the gate"](../00-context/harness-research.md#identity-and-the-gate) for using this with a runner-verified agent identity (DID / token) on `_ctx`.
