...
**One additive library change — the `flags` primitive (backward-compatible, opt-in).** Everything else below is tests, docs, and CI. No breaking change; an unset `flags` config is a no-op, so existing gates behave identically.
