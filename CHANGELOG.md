...
### Tests
- **+16 (suite 197 → 213; typecheck clean):** `test/budget-pricing-contract.test.js` (+7) — unpriced accrues no cost but still accrues tokens; priced/absent rounds accrue normally (back-compat); default-off emits the loud `unpriced` phase and never halts; `failClosedOnUnpriced` + finite cap halts at `check()` with `rule: "budget.unpriced"`; no-cost-cap makes it a pure audit signal (no halt); per-round clearing (a later priced round lifts the halt); the token cap still enforces across all-unpriced rounds. `test/budget-float-accrual.test.js` (+4) — the float-drift cap-bypass guards. `test/budget-input-hardening.test.js` (+5) — null-without-flag, NaN/±Inf cap-poison, negative cost/token refund evasion, and the non-cost-action regression.
